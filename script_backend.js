document.addEventListener('DOMContentLoaded', function() {
    ...
    function animateCounter(element, target) {
        window.__countersAnimated = true;
        let current = 0;
    ...
    const I18N = {
        es: {
            ...
            'tl-1-t': 'Fundamentos (2025)',
            'tl-1-d': 'Aprendizaje autodidacta: HTML, CSS y JavaScript; lógica, algoritmos y primeros proyectos.',
            'tl-2-t': 'Especialización Backend (2025)',
            'tl-2-d': 'APIs REST con Node.js y Express, autenticación JWT y bases de datos SQL/NoSQL.',
            'tl-3-t': 'DevOps básico (2025)',
            'tl-3-d': 'Docker, CI/CD con GitHub Actions y despliegues en servicios gestionados (Render, Neon).',
            'tl-4-t': 'Proyectos full-stack desplegados (2025 – 2026)',
            'tl-4-d': 'Monorepo e-commerce con Stripe, clon de streaming y acortador de URLs, todos desplegados y en vivo.',
            'tl-5-t': 'Siguiente etapa: microservicios',
            'tl-5-d': 'En construcción: arquitectura de microservicios con APIs independientes, colas de mensajes y observabilidad.',
        ...
        en: {
            ...
            'tl-1-t': 'Fundamentals (2025)',
            'tl-1-d': 'Self-taught: HTML, CSS and JavaScript; logic, algorithms and first projects.',
            'tl-2-t': 'Backend specialization (2025)',
            'tl-2-d': 'REST APIs with Node.js and Express, JWT auth and SQL/NoSQL databases.',
            'tl-3-t': 'Basic DevOps (2025)',
            'tl-3-d': 'Docker, CI/CD with GitHub Actions and deployments on managed services (Render, Neon).',
            'tl-4-t': 'Deployed full-stack projects (2025 – 2026)',
            'tl-4-d': 'E-commerce monorepo with Stripe, streaming clone and URL shortener, all deployed and live.',
            'tl-5-t': 'Next stage: microservices',
            'tl-5-d': 'Under construction: microservices architecture with independent APIs, message queues and observability.',
            'soft-title': '// soft_skills',
            'soft-1': 'Teamwork',
    ...
    function applyGitHubCounters(projects, commits, stars) {
        const values = [projects, null, commits, stars];
        document.querySelectorAll('.about-stats .stat-card').forEach((card, index) => {
            ...
            if (counter) {
                ...
                if (window.__countersAnimated) counter.textContent = value;
            }
            const codeNum = card.querySelector('.code-number');
            if (codeNum) codeNum.textContent = value;
        });
    }
            ...
            .then(([user, repos]) => {
                ...
                return Promise.all(repoList.map(repo =>
                    fetch(`https://api.github.com/repos/${GITHUB_USER}/${repo.name}/commits?per_page=1`)
                        .then(res => {
                            if (res.status === 404 || res.status === 409 || res.status === 422) return 0;
                            if (!res.ok) return 0;
                            const last = totalCommitsFromLink(res.headers.get('Link'));
